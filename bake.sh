...
checkExist () {
  if ${PWD##*/} == 'puppet-code-challenge'; then
    echo 'Code challenge already exists locally'
  else
    docker run -ti --rm -v ${HOME}:/root -v $(pwd):/git alpine/git https://github.com/brokorus/puppet-code-sample.git
    cd puppet-code-sample
  fi
